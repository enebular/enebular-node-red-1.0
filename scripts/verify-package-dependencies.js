...
function verifyDependencies() {
  ...
  packages.forEach((package) => {
    let modulePackage = require(path.join(
    ...
    dependencies.forEach((module) => {
      try {
        if (!/^@uhuru\/enebular-node-red-/.test(module)) {
          should.exist(
            rootDependencies[module],
            `[${package}] '${module}' missing from root package.json`
          );
          try {
            rootDependencies[module].should.eql(
              modulePackage.dependencies[module],
              `[${package}] '${module}' version mismatch. Expected '${modulePackage.dependencies[module]}' (got '${rootDependencies[module]}') `
            );
          } catch (err) {
            if (fixFlag) {
